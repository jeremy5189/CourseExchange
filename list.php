            ...
            <div class="row-fluid">
                <div class="span12">
                    <div class="hero-unit">
                      ...
                      <p>
                        <?
                        echo "<table class=\"table table-bordered\"><thead><tr><th>學校</th><th>姓名</th><th>選課代號</th><th>課程名稱</th><th>換</th><th>選課代號</th><th>課程名稱</th></tr></thead><tbody><tr>";
                        $getclass = mysql_pconnect("localhost" , DB_USER ,DB_PASS ) or die ("無法連接".mysql_error()); 
                        mysql_select_db(course_exchange, $getclass) or die ("無法選擇資料庫".mysql_error()); 
                        $classquery = mysql_query("SELECT * FROM `ce_course` LIMIT 0, 30");
                        while($row = mysql_fetch_assoc($classquery)){  //撈出資料
                          ...
                          echo '<td rowspan="2">'.$row['university']."</td><td>".$row['name']."</td><td>".$row['changeID']."</td><td>".$row['changeName']."</td><td>"." "."</td><td>".$row['wantID']."</td><td>".$row['wantName']."</td><tr><tbody>";
